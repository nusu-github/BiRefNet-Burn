...
mod tests {
    ...
    fn erfinv_matches_pytorch_reference_values() {
        ...
        // Test zero value
        assert!((data.iter::<f64>().next().unwrap() - 0.0).abs() < 1e-6);

        // Test central range value (|y| <= 0.7)
        let expected_half = 0.4769_f64; // Expected for erfinv(0.5)
        assert!((data.iter::<f64>().nth(1).unwrap() - expected_half).abs() < 1e-3);

        // Test outer range value (|y| > 0.7)
        let expected_nine = 1.1631_f64; // Expected for erfinv(0.9)
        assert!((data.iter::<f64>().nth(2).unwrap() - expected_nine).abs() < 1e-2);
    }
}
