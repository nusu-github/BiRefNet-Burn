//! Model and component configurations for BiRefNet.
//!
//! This module defines a set of hierarchical configuration structs (`#[derive(Config)]`)
//! that control every aspect of the BiRefNet model, from the choice of backbone
//! to the specifics of the decoder and refinement modules.
//!
//! The main entry point is `ModelConfig`, which aggregates all other configurations.
//! These configurations are designed to be deserializable from a file (e.g., TOML, JSON)
//! and provide default values for easy setup.
//!
//! The module also includes validation logic (`ModelConfig::validate`) to ensure that
//! the provided configuration is logically consistent and supported by the current
//! implementation, mirroring the constraints of the original PyTorch version.
