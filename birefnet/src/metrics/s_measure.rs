...
pub fn calculate_s_measure<B: Backend>(
    ...
    if y == 0.0 {
        // All background
        1.0 - pred.mean().into_scalar().elem::<f32>()
    } else if y == 1.0 {
        // All foreground
...
fn calculate_s_object<B: Backend>(pred: Tensor<B, 2>, gt: Tensor<B, 2>) -> f32 {
    ...
    let count = mask.clone().float().sum().into_scalar().elem::<f32>();
    if count == 0.0 {
        return 0.0;
    }

    let x = masked_pred.sum().into_scalar().elem::<f32>() / count;
...
fn calculate_centroid<B: Backend>(gt: Tensor<B, 2>) -> (usize, usize) {
    ...
    if count.elem::<f32>() == 0.0 {
        ...
        let cy = (y_grid * mask.clone().float())
            .sum()
            .into_scalar()
            .elem::<f32>()
            / count.elem::<f32>();
        let cx = (x_grid * mask.float()).sum().into_scalar().elem::<f32>() / count.elem::<f32>();
...
fn calculate_region_weights(
    ...
    let w1 = (cx * cy) as f32 / area;
    let w2 = (cy * (width - cx)) as f32 / area;
    let w3 = ((height - cy) * cx) as f32 / area;
    let w4 = 1.0 - w1 - w2 - w3;
...
fn calculate_ssim<B: Backend>(pred: Tensor<B, 2>, gt: Tensor<B, 2>) -> f32 {
    ...
    let sigma_y_sq = (gt_centered
        ...
        / (n - 1.0).max(1.0))
    .elem::<f32>();
    let sigma_xy = (pred_centered * gt_centered)
        .sum()
        .into_scalar()
        .elem::<f32>()
        / (n - 1.0).max(1.0);
