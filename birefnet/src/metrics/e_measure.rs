...
impl<B: Backend> Metric for EMeasureMetric<B> {
    ...
    fn update(&mut self, input: &Self::Input, _metadata: &MetricMetadata) -> MetricEntry {
        ...
        for b in 0..batch_size {
            ...
            self.state
                .changeable_ems
                .push(changeable_em.into_iter().map(|v| v as f64).collect());
        }
...
pub fn calculate_e_measure<B: Backend>(
    ...
    let gt_size = ((height * width) as f32).elem::<B::FloatElem>();
...
fn calculate_em_with_threshold<B: Backend>(
    ...
    let bg_numel = gt_size.elem::<f32>() - fg_numel;

    let enhanced_matrix_sum = if gt_fg_numel.elem::<f32>() == 0.0 {
        bg_numel
    } else if gt_fg_numel.elem::<f32>() == gt_size.elem::<f32>() {
        fg_numel
    } else {
    ...
    enhanced_matrix_sum / (gt_size.elem::<f32>() - 1.0 + 1e-8)
}
...
fn calculate_em_with_cumsumhistogram<B: Backend>(
    ...
    for threshold in 0..num_bins {
        ...
        changeable_ems[threshold] = enhanced_matrix_sum / (gt_size.elem::<f32>() - 1.0 + 1e-8);
    }
